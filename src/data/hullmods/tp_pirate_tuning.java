...
public class tp_pirate_tuning extends BaseHullMod {

	private static Map speed = new HashMap();
	static {
		speed.put(HullSize.FRIGATE, 16.6f);
		speed.put(HullSize.DESTROYER, 10f);
		speed.put(HullSize.CRUISER, 6.6f);
		speed.put(HullSize.CAPITAL_SHIP, 3.3f);
	}
...
//	}
	...
	private static final float PEAK_MULT = 0.78f;
	//private static final float CR_DEG_MULT = 2f;
	//private static final float OVERLOAD_DUR = 50f;
	private static final float FLUX_DISSIPATION_MULT = 1.33f;
	//private static final float FLUX_CAPACITY_MULT = 1f;
	
	private static final float RANGE_THRESHOLD = 775f;
	private static final float RANGE_MULT = 0.25f;
    private static final float ZERO_FLUX_MULT = 33f;
...
//		stats.getEngineMalfunctionChance().modifyFlat(id, MALFUNCTION_PROB);
		...
		stats.getVentRateMult().modifyMult(id, 0.66f);
...
//		stats.getRecoilPerShotMult().modifyMult(id, RECOIL_MULT);
	...
	public String getDescriptionParam(int index, HullSize hullSize) {
		...
		if (index == 4) return "0.78";
		if (index == 5) return Misc.getRoundedValue(FLUX_DISSIPATION_MULT);
...
//				ship.getEngineController().isStrafingRight()) {
			ship.getEngineController().fadeToOtherColor(this, color, null, 1f, 0.133f);
			ship.getEngineController().extendFlame(this, 0.083f, 0.083f, 0.083f);
//		}
	}
}
